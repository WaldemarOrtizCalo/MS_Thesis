...
# Export log
write_csv(as.data.frame(hr_log),
          "1.DataManagement/ch2_data/clean/homerange_polygons/north/north_HR_log.csv")
...
# Deer Locations
locs <- locs_deer %>% filter(site == "South")

# Covariate Layers/Stack 
cov_layers <- rast(covlayers_south)
...
hr_log <- foreach(i = 1:nrow(valid_homeranges),
                  ...
                  .errorhandling = "pass") %dopar% {
                    ...
                    # Export
                    sf_export <- bind_rows(KDE_95,AKDE_95) %>% 
                      st_write(paste0("1.DataManagement/ch2_data/clean/homerange_polygons/south/",
                                      indiv_id,"_",interval_id,".shp"),
                               append = F,
                               quiet = T)
...
# Export log
write_csv(as.data.frame(hr_log),
          "1.DataManagement/ch2_data/clean/homerange_polygons/south/south_HR_log.csv")

#      Covariate Extraction                                                 ####

# List of Home Ranges
polygon_filepaths <- list.files("1.DataManagement/ch2_data/clean/homerange_polygons/south",
                                pattern = ".shp",
                                full.names = T)
                   ...
                   .errorhandling = "pass") %do% {
                     ...
                     covs <- foreach(j = 1:nrow(HR_estimates),
                                     ...
                                     .errorhandling = "pass") %do% {
                                       ...
                                       mean_vals <- global(cropped_landscape[[c("south_dem",
                                                                                "slope",
                                                                                "aspect",
                                                                                "TRI")]],"mean", na.rm = T) %>% 
                                         rownames_to_column(var = "cov_name") %>% as_tibble %>% 
                                         pivot_wider(names_from = cov_name, 
                                                     values_from = mean) %>% 
                                         rename(dem = south_dem)
                                       
                                       contag <- lsm_l_contag(cropped_landscape[["south_nlcd"]]) %>% 
                                         dplyr::select(c(metric,value)) %>% 
                                         pivot_wider(names_from = metric, 
                                                     values_from = value)
                                       
                                       lsi <- lsm_l_lsi(cropped_landscape[["south_nlcd"]]) %>% 
                                         dplyr::select(c(metric,value)) %>% 
                                         pivot_wider(names_from = metric, 
                                                     values_from = value)
                                       
                                       shdi <- lsm_l_shdi(cropped_landscape[["south_nlcd"]]) %>% 
                                         dplyr::select(c(metric,value)) %>% 
                                         pivot_wider(names_from = metric, 
                                                     values_from = value)
                                       
                                       mean_patcharea <- lsm_c_area_mn(cropped_landscape[["south_nlcd"]]) %>% 
                                         mutate(class = as.character(class)) %>% 
                                         left_join(reclass_matrixSouth) %>% 
                                         dplyr::select(c(class_name, value)) %>% 
                                         pivot_wider(names_from = class_name,
                                                     values_from = value,
                                                     names_prefix = "meanpatcharea_")
                                       
                                       patch_density <- lsm_c_pd(cropped_landscape[["south_nlcd"]]) %>% 
                                         mutate(class = as.character(class)) %>% 
                                         left_join(reclass_matrixSouth) %>% 
                                         dplyr::select(c(class_name, value)) %>% 
                                         pivot_wider(names_from = class_name,
                                       ...
                                       landcover_proportions <- lsm_c_ca(cropped_landscape[["south_nlcd"]])%>% 
                                         mutate(class = as.character(class)) %>% 
                                         left_join(reclass_matrixSouth) %>% 
                                         dplyr::select(c(class_name, value)) %>% 
                                         mutate(value = value/hr_size$hr_size_ha) %>% 
...
write_csv(HR_covs,
          "1.DataManagement/ch2_data/clean/cov_extractions/south_HR_covs.csv")
